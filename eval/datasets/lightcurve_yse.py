...
true), but — same move as the reference spectra-classification harness's `render_spectrum_plot` —
`render_lightcurve_plot` below turns one object's `atcat_*` photometry into an ordinary flux-vs-time
PNG scatter plot, which the base model's native-vision pathway *can* consume, same as
...
# Digit-code label mapping, SN_LABELS order — same format `eval.datasets.image_galaxy10.
# CLASS_CODES` uses and for the same confirmed-live reason (eval/prompt_playground.py, see that
# module's docstring): a short, closed digit-code answer is far more reliably emitted and parsed
# than free-text label-name matching. `eval.metrics.caption_to_label.make_predictor` is already
# dataset-agnostic, so this plugs into the exact same scoring path as the image track.
...
# Structurally the same instruction shape as the reference spectra-classification harness's
# prompts ("classify into one of the following categories: <options>... conclude with the exact
# format") — a fixed-vocabulary classifier instruction naming every allowed category — just in
# the digit-code form confirmed empirically superior for compliance (see module docstring above
# and eval.datasets.image_galaxy10.CLASS_CODE_PROMPT's docstring for the same finding on the
# image track). Ending mid-sentence matters for the equipped side, which never goes through a
# chat template (see generate_caption's docstring) — a genuine raw-text continuation, not styling.
...
def render_lightcurve_plot(row: pd.Series) -> Image.Image:
    ...
    base-side contract), same role `render_spectrum_plot` plays for the spectra track in the
    reference harness. Masks by `atcat_use` first (the excluded-i sentinel at `atcat_band_id=0` is
    only safe to drop via this mask — see `captioner.data.transients_dataset`'s module docstring),
    then plots g/r separately by color so the model has some chance of reading band information
    off the picture the same way it would off two colored curves in a real light-curve plot.
    """
    import matplotlib
...
def stratified_sample(
    ...
    stratified_sample` (see that function's docstring for the full mechanism) — reimplemented
    here rather than imported/shared since it's keyed on `SN_LABELS`, not `GALAXY10_LABELS`, and
    the two datasets' loaders are already deliberately independent (see module docstring re:
    `load_lightcurve_table`). One `numpy.random.default_rng(seed)`, consumed in `SN_LABELS`'
    fixed order, so the same seed always draws the same objects; raises rather than silently
    returning a bigger-or-smaller-than-requested sample if `n_total` is infeasible.
    """
